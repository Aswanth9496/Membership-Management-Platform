  ...
  try {
    ...
    if (adminToken) {
      try {
        ...
        // Fall back to check memberToken if admin token is invalid
        console.warn('Admin token invalid, checking member token if present');
      }
    }

    if (memberToken) {
      const decoded = verifyToken(memberToken);
      if (decoded) {
        const member = await User.findById(decoded.id).select('-password');
    ...
    throw new ApiError(401, 'Invalid or expired tokens. Please login again');
  } catch (error) {
    next(error);
  }
};
