...
import { loginMemberUser } from '../../features/auth/authThunks';
...
export default function Login() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Dispatch the login thunk for member
        const resultAction = await dispatch(loginMemberUser(formData));

        if (loginMemberUser.fulfilled.match(resultAction)) {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 w-full max-w-md my-8">
            <h1 className="text-2xl font-bold text-slate-900 mb-1 tracking-tight">Agency Login</h1>
            <p className="text-sm text-slate-500 mb-6">Sign in to access your travel agency portal.</p>
